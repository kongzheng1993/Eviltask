...
@Component
@Scope("prototype")
public class SftpDownloadTaskImpl extends AbsSftpTask {

    private static final Logger logger = LoggerFactory.getLogger(SftpDownloadTaskImpl.class);
    ...
    public boolean doTask() {
        ...
        String localFilePath = org.springframework.util.StringUtils.isEmpty(sftpCfg.getSftpLocalPath()) ? taskLocalDir + task.getTaskName() + "/" + sftpCfg.getSftpAddr() + "/" : sftpCfg.getSftpLocalPath();

        boolean result = SftpUtils.downloadFiles(channelSftp, sftpCfg.getSftpRemotePath(), localFilePath, sftpCfg.getSftpEncoding(), sftpCfg.getRecursion());
